...
d_inner_hid=2048 # 修正: d_inner -> d_inner_hid
d_k=64
d_v=64
optimizer=adam # optimizer
scheduler=cosLR # type of lr scheduler
dropout=0.1
lr=1e-4
epoch=40
eval_epoch=41 # number of epoch we want to start to evaluate
normalize=log # 修正: True -> log (参考 smurf_financial.sh)
loss_lambda=5 # 新增: 辅助损失权重 (参考 smurf_financial.sh)
...
save_name=financial_fthp_model_best_${normalize}_lambda_${loss_lambda}
...
# --- 评估命令 ---
load_path_name=./checkpoints/financial/${save_name}.pth
save_result=./results/financial/${save_name}_samples
